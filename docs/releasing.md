...
1. Update versions and CHANGELOG.md (a `## X.Y.Z - date` section becomes the
   ...
   `bash js/@corbet-labs/cgreet/scripts/sync-assets.sh`. Commit to `main`.
...
3. Push the tag `vX.Y.Z` for the version in `Cargo.toml`. The tag push is the
   ...
   - `prepare` (read-only, no credentials) calls `ci.yml` at the tag with
     `metadata,release-config,rust,javascript,rust-package,js-package,jsr-package,python-package,typst-package`.
...
Until npm and PyPI trust `corbet-foss/cgreet:release.yml`, upload them from the
...
```sh
tag=vX.Y.Z repo=corbet-foss/cgreet rev=27c248aefa3c7198be6716a884d290c717774b21
