/**
 * TypeScript side of the cross-language conformance gate: runs every
 * `tests/vectors/*.json` vector through `src/index.ts` and compares with
 * `expected` exactly. Exits non-zero with the first mismatch.
 *
 * Run from the package root:
 *   bun ./scripts/conformance.mts
 */
import { readdirSync, readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join, resolve } from 'node:path';
import {
    deHonorificWarning,
    deSalutation,
    parseRegion,
    recipientSalutationWarning,
    regionUsesComma,
    salutationHonorific,
    salutationLastName,
    salutationSurname,
    salutationTitles,
    type Region,
} from '../src/index.js';

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '../../../..');

interface Vector {
    name: string;
    fn: string;
    input: string;
    region?: string;
    location?: string;
    expected: unknown;
}

function checkEqual(actual: unknown, expected: unknown, what: string): void {
    const got = JSON.stringify(actual) ?? 'undefined';
    const want = JSON.stringify(expected) ?? 'undefined';
    if (got !== want) throw new Error(`${what}: ${got} vs ${want}`);
}

function runVector(file: string, vector: Vector): void {
    const what = `${file} :: ${vector.name}`;
    let actual: unknown;
    switch (vector.fn) {
        case 'region_parse':
            actual = parseRegion(vector.input);
            break;
        case 'region_uses_comma': {
            const region = parseRegion(vector.input);
            if (region === null) throw new Error(`${what}: unknown region`);
            actual = regionUsesComma(region);
            break;
        }
        case 'salutation_last_name':
            actual = salutationLastName(vector.input);
            break;
        case 'salutation_honorific':
            actual = salutationHonorific(vector.input);
            break;
        case 'salutation_surname':
            actual = salutationSurname(vector.input);
            break;
        case 'salutation_titles':
            actual = salutationTitles(vector.input);
            break;
        case 'de_salutation': {
            if (vector.region === undefined) throw new Error(`${what}: missing region`);
            const region = parseRegion(vector.region) as Region | null;
            if (region === null) throw new Error(`${what}: unknown region`);
            actual = deSalutation(vector.input, region);
            break;
        }
        case 'recipient_salutation_warning':
            if (vector.location === undefined) throw new Error(`${what}: missing location`);
            actual = recipientSalutationWarning(vector.location, vector.input);
            break;
        case 'de_honorific_warning':
            if (vector.location === undefined) throw new Error(`${what}: missing location`);
            actual = deHonorificWarning(vector.location, vector.input);
            break;
        default:
            throw new Error(`${what}: unknown fn ${vector.fn}`);
    }
    checkEqual(actual, vector.expected, what);
}

const dir = join(ROOT, 'tests/vectors');
const files = readdirSync(dir)
    .filter((file) => file.endsWith('.json'))
    .sort();
if (files.length === 0) throw new Error('no vector files in tests/vectors');
let count = 0;
for (const file of files) {
    const vectors = JSON.parse(readFileSync(join(dir, file), 'utf8')) as Vector[];
    for (const vector of vectors) {
        runVector(file, vector);
        count += 1;
    }
}
console.log(`TS conformance green: ${count} vectors across ${files.length} files`);
